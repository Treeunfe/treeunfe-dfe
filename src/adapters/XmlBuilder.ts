...
class XmlBuilder {
  ...
  assinarXML(xml: string, tagAssinar: string) {
    ...
    signedXmlObj.signatureAlgorithm =
  ...
  buildSoapEnvelope(
    ...
    const soapNamespace =
      soapNamespaces[soapVersion] || soapNamespaces["soap12"];
    ...
    let soapEnvXml = this.serializeXml(
      soapEnvelopeObj,
      `${soapVersion}:Envelope`
    );
