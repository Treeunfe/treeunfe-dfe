...
function filterFieldsByType<T extends object>(
  source: any,
...
export const mountCOFINS = (cofins: dadosCOFINS) => {
  ...
  if (!cofinsKey) {
    console.warn(
      `Código de situação tributária do COFINS ${cst_cofins} não é reconhecido. Alterado para padrão 'COFINSAliq'.`
    );
...
export const mountPIS = (pis: dadosPIS): PIS => {
  ...
  if (!pisKey) {
    console.warn(
      `Código de situação tributária do PIS ${cst_pis} não é reconhecido. Alterado para padrão 'PISAliq'.`
    );
    pisKey = "PISAliq";
  }
...
export const mountICMS = (icms: dadosICMS): ICMS => {
  ...
  if (!icmsKey) {
    console.warn(
      `Código de situação tributária ${cod_sit} não é reconhecido. Alterado para padrão 'ICMS00'.`
    );
    icmsKey = "ICMS00";
  }
