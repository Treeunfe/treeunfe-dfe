...
class Utility {
  ...
  calcularHashCSRT = (CSRT: string, chaveAcesso: string): string => {
    const entrada = CSRT + chaveAcesso;

    return createHash("sha1").update(entrada, "utf8").digest("base64");
  };
  ...
  getWebServiceUrl(
    ...
    let { chaveMae, chaveFilha } = this.setAmbiente(
      metodo,
      ambienteNacional,
      versao,
      mod
    ...
    if (!url) {
      throw new Error(
        `Não foi possível recuperar a url para o webservice: ${chaveFilha}`
      );
    }
    return url;
  }
  ...
  getUrlNFCe(metodo: string, ambienteNacional = false, versao = ""): string {
    ...
    const url =
      urls[chaveMae] && this.getLatestURLConsulta(urls[chaveMae], metodo);
    if (!url) {
      throw new Error(
        `Não foi possível recuperar a url para consulta de NFCe: ${chaveMae}`
      );
    }
    return url;
  }
        ...
        if (isValid) {
          ...
          reject({
            ...
            message: this.formatErrorMessage(
              xmlDoc.validationErrors[0].message
            ),
          });
        }
      } catch (error: any) {
  ...
  verificaRejeicao(data: string, metodo: string, name?: string) {
    ...
    for (const xMotivo of allXMotivos) {
      if (
        xMotivo &&
        (xMotivo.includes("Rejeição") || xMotivo.includes("Rejeicao"))
      ) {
    ...
    if (
      infProt &&
      (infProt?.xMotivo?.includes("Rejeição") ||
        infProt?.xMotivo?.includes("Rejeicao"))
    ) {
  ...
  getProtNFe(xmlRetorno: string): {
    ...
    xml2js.parseString(xmlRetorno, (err, result) => {
      if (err) {
        console.error(
          "Erro ao parsear o XML para captura do nRec e protNFe:",
          err
        );
      } else {
  ...
  private getRequestLogFileName(metodo: string, tipo: string) {
    switch (metodo) {
      ...
      default:
        throw new Error(
          "Erro: Requisição de nome para método não implementado."
        );
    }
  }
  ...
  salvaConsulta(
    ...
    const {
      armazenarXMLConsulta,
      pathXMLConsulta,
      armazenarXMLConsultaComTagSoap,
    } = this.environment.config.dfe;
    const xmlConsultaASalvar = armazenarXMLConsultaComTagSoap
      ? xmlFormated
      : xmlConsulta;
  ...
  salvaRetorno(
    xmlRetorno: string,
    responseInJson: GenericObject | undefined,
    metodo: string,
    name?: string
  ) {
    const fileName = name || this.getRequestLogFileName(metodo, "retorno");
    const { armazenarXMLRetorno, pathXMLRetorno, armazenarRetornoEmJSON } =
      this.environment.config.dfe;
