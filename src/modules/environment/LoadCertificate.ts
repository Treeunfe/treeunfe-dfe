...
class LoadCertificate {
          ...
          async (error, result) => {
            if (error) {
              if (error.message.toLowerCase().includes("mac verify error")) {
                return reject(
                  new Error("Erro ao ler o certificado: Senha incorreta.")
                );
              } else {
                return reject(
                  new Error(`Erro ao ler o certificado: ${error.message}`)
                );
              }
            }
            ...
            const now = new Date();
            if (
              now < certForge.validity.notBefore ||
              now > certForge.validity.notAfter
            ) {
              return reject(
                new Error(
                  "Erro ao carregar o certificado: O certificado fornecido expirou ou ainda não é válido."
                )
              );
            }
  ...
  private loadCertificateWithNodeForge(): Promise<CertificateLoadResult> {
    return new Promise((resolve, reject) => {
      try {
        ...
        if (!key) {
          return reject(
            new Error("Erro ao carregar chave privada do certificado.")
          );
        }
        ...
        const certForge = forge.pki.certificateFromPem(
  ...
  async loadCertificate(): Promise<CertificateLoadReturn> {
    ...
    if (
      this.config.lib?.useOpenSSL ||
      this.config.lib?.useOpenSSL === undefined
    ) {
