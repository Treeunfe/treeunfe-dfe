...
class ValidateEnvironment {
  checkRequiredSettings(config: TreeunfeNFe) {
    ...
    try {
      const requiredConfigFields: { [K in keyof TreeunfeNFe]: string[] } = {
      ...
      (Object.keys(requiredConfigFields) as (keyof TreeunfeNFe)[]).forEach(
        (categoryKey) => {
          ...
          if (!category) {
            ...
            tableData.push({
              ...
              Faltando: `Chave principal faltando`,
            });
          } else {
            ...
            if (fields) {
              ...
              if (missingConfig && missingConfig.length > 0) {
                errors.push(
                  `Configurações faltando em '${categoryKey}': [${missingConfig.join(
                    ", "
                  )}].`
                );
                tableData.push({
