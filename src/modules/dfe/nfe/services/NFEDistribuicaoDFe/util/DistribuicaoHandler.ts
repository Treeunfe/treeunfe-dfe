...
class DistribuicaoHandler {
  ...
  protected salvaArquivos(
    XMLDistribuicaoInJson: GenericObject,
    XMLDistribuicao: string,
    chNFe: string
  ): void {
    const {
      pathXMLDistribuicao,
      baixarXMLDistribuicao,
      armazenarRetornoEmJSON,
  ...
  deCompressDFeXML(
    loteDistDFeInt: AxiosResponse<any, any>,
    metodo: string,
    xmlConsulta: string
  ) {
    ...
    xml2js.parseString(loteDistDFeInt, (err, result) => {
      ...
      docZips.forEach((docZip: any) => {
        ...
        const xmlDistribuicaoInJson = json.convertXmlToJson(
          cleanedXml,
          `${metodo}_${tipo}`,
          nsu
        );
