...
class NFERecepcaoEventoService
  extends BaseNFE
  implements NFERecepcaoEventoServiceImpl
{
  tpEvento: string;
  modelo?: string;
  ...
  private separaEventosPorAmbiente(evento: TipoEvento[]) {
    ...
    const nacional = evento.filter((event) =>
      ["210210", "210200", "210220", "210240", "110140"].includes(
        event.tpEvento
      )
    );
    const regional = evento.filter(
      (event) =>
        !["210210", "210200", "210220", "210240", "110140"].includes(
          event.tpEvento
        )
  ...
  private gerarXmlRecepcaoEvento(
    ...
    for (let i = 0; i < evento.length; i++) {
      ...
      const eventoXML = this.xmlBuilder.gerarXml(
        eventoObject,
        "evento",
        this.metodo
      );
  ...
  private trataRetorno(responseInJson: GenericObject) {
    ...
    if (retornoEventos instanceof Array) {
      for (let i = 0; i < retornoEventos.length; i++) {
        ...
        const tipoEvento = this.getTipoEventoName(
          retornoEventos[i].infEvento.tpEvento
        );
    ...
    const tipoEvento = this.getTipoEventoName(
      retornoEventos.infEvento.tpEvento
    );
    this.xMotivoPorEvento.push({
      chNFe,
      xMotivo,
  ...
  protected async enviaEvento(
    ...
    try {
      ...
      xmlConsulta = this.gerarXmlRecepcaoEvento(
        evento,
        idLote,
        ambienteNacional
      );
      ...
      const { xmlFormated, agent, webServiceUrl, action } =
        await this.gerarConsulta.gerarConsulta(
          xmlConsulta,
          this.metodo,
          ambienteNacional || this.isAmbienteNacional(this.tpEvento),
          "",
          this.modelo
        );
      ...
      const fileName = ambienteNacional
        ? "RecepcaoEvento[Nacional]-consulta"
        : "RecepcaoEvento[Regional]-consulta";
      this.utility.salvaConsulta(
        xmlConsulta,
        xmlConsultaSoap,
        this.metodo,
        fileName
  ...
  async Exec(data: EventoNFe) {
    ...
    let responseNacionalInJson,
      responseRegionalInJson = null;
    let finalResponseInJson = [];

    if (nacional.length > 0) {
      const retornoNacional = await this.enviaEvento(nacional, idLote, 0);

      responseNacionalInJson = this.utility.verificaRejeicao(
        retornoNacional,
        this.metodo
      );

      this.utility.salvaRetorno(
        retornoNacional,
        responseNacionalInJson,
        this.metodo,
        "RecepcaoEvento[Nacional]-retorno"
      );
    ...
    if (regional.length > 0) {
      ...
      responseRegionalInJson = this.utility.verificaRejeicao(
        retornoRegional,
        this.metodo
      );
      ...
      this.utility.salvaRetorno(
        ...
        this.metodo,
        "RecepcaoEvento[Regional]-retorno"
      );
