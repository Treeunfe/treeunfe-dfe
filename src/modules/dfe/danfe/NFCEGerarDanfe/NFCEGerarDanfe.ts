...
class NFCEGerarDanfe {
  ...
  constructor(props: NFEGerarDanfeProps) {
    ...
    const { det, ide, emit, dest, total, transp, pag, infAdic } =
      nfeData.infNFe;
  ...
  _buildHeader() {
    ...
    const _buildIdentificacaoEmit = () => {
      const centeredPosEmit = this.centeredPos(
        `CNPJ: ${documento} ${this.emit.xNome}`
      );
      const centeredPosEnd = this.centeredPos(identificationJoined);
      const centeredPosText = this.centeredPos(
      ...
      this.doc
        ...
        .text(`CNPJ: ${documento} `, centeredPosEmit, 2, {
          lineBreak: false,
        })
        .font("Arial-bold")
        .text(this.emit.xNome)
        .fontSize(this.fontSize)
        .font("Arial")
  ...
  _buildProdutos() {
    ...
    const header = (top: number) => {
      ...
      this.doc
        .font("Arial-bold")
        .fontSize(this.fontSize)
        .text("Código", x, top, { width: columnWidths.codigo });
  ...
  _buildTotais() {
    ...
    const quantidadeTotalDeItens = Array.isArray(this.det)
      ? this.det.length
      : 1;
    ...
    if (Array.isArray(this.det)) {
      ...
      valTotal = this.det.reduce(
        (sum, item) => sum + parseFloat(item.prod.vProd),
        0
      );

      // Calcula o total dos acréscimos
      acrescimo = this.det.reduce(
        (sum, item) =>
          sum +
      ...
      desconto = this.det.reduce(
        (sum, item) => sum + parseFloat(item.prod.vDesc || "0"),
        0
      );
    } else {
      ...
      acrescimo =
        parseFloat(this.det.prod.vFrete || "0") +
        parseFloat(this.det.prod.vSeg || "0") +
        parseFloat(this.det.prod.vOutro || "0");
      desconto = parseFloat(this.det.prod.vDesc || "0");
    }
    const valorTotal = valTotal.toLocaleString("pt-BR", {
      minimumFractionDigits: 2,
      maximumFractionDigits: 3,
    });
    const acrescimoTotal = acrescimo.toLocaleString("pt-BR", {
      minimumFractionDigits: 2,
      maximumFractionDigits: 3,
    });
    const descontoTotal = desconto.toLocaleString("pt-BR", {
      minimumFractionDigits: 2,
      maximumFractionDigits: 3,
    });
    ...
    this.doc.text(
      ...
      this.calculaPosicao(String(quantidadeTotalDeItens)),
      tableTop,
      {
        align: "right",
      }
    ...
    if (desconto > 0) {
      ...
      this.doc.text(
        descontoTotal,
        this.calculaPosicao(descontoTotal),
        tableTop,
        {
          align: "right",
        }
    ...
    if (acrescimo > 0) {
      ...
      this.doc.text(
        acrescimoTotal,
        this.calculaPosicao(acrescimoTotal),
        tableTop,
        {
          align: "right",
        }
    ...
    if (desconto > 0 || acrescimo > 0) {
      const totalPagar = parseFloat(
        String(valTotal + acrescimo - desconto)
      ).toLocaleString("pt-BR", {
        minimumFractionDigits: 2,
        maximumFractionDigits: 3,
      });
    ...
    if (Array.isArray(this.pag.detPag)) {
      ...
      this.doc.text(
        this.pag.detPag.xPag || "Não informado",
        2,
        topTiposPag + this.itemHeight
      );
    }
    ...
    this.doc.text(
      "VALOR PAGO R$",
      this.calculaPosicao("VALOR PAGO R$"),
      tableTop,
      {
        align: "right",
      }
    ...
    if (Array.isArray(this.pag.detPag)) {
      for (let pagto of this.pag.detPag) {
        const val = parseFloat(pagto.vPag).toLocaleString("pt-BR", {
          minimumFractionDigits: 2,
          maximumFractionDigits: 3,
        });
        this.doc.text(
          val,
          this.calculaPosicao(val),
      ...
      const val = parseFloat(this.pag.detPag.vPag).toLocaleString("pt-BR", {
        minimumFractionDigits: 2,
        maximumFractionDigits: 3,
      });
      this.doc.text(
    ...
    let valTroco = 0;
    if (Array.isArray(this.pag.detPag)) {
      valTroco = this.pag.detPag.reduce(
        (sum, item) => sum + parseFloat(item.vTroco || "0"),
        0
      );
    } else {
    ...
    const troco = valTroco.toLocaleString("pt-BR", {
      minimumFractionDigits: 2,
      maximumFractionDigits: 3,
    });
  ...
  _buildFooter(qrCodeBuffer: Buffer) {
    ...
    this.doc
      .font("Arial-bold")
      .text("Consulte pela Chave de Acesso em", 0, tableTop, {
        align: "center",
      });
    tableTop += this.itemHeight;
    ...
    this.doc
      .font("Arial-bold")
      .text(
        `NFC-e nº ${this.ide.nNF} Série ${this.ide.serie} ${dtaEmi}`,
        75,
        tableTop,
        {
          align: "left",
    ...
    this.doc.text(
      `Tributos Totais Incidentes (Lei Federal 12.741/2012): R$ ${parseFloat(
        this.total.ICMSTot.vTotTrib || "0"
      ).toFixed(2)}`,
      0,
      topBeforeQrCode,
      {
  ...
  async generatePDF(exibirMarcaDaguaDanfe?: boolean) {
    try {
      ...
      const qrCodeBuffer = await this.getQRCodeBuffer(
        this.infNFeSupl?.qrCode || ""
      );
