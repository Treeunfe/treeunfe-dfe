...
import {
  generateQRCodeURLOffline,
  generateQRCodeURLOnline,
} from "./util/NFCEQRCode.js";

class NFCEAutorizacaoService
  extends BaseNFE
  implements NFCEAutorizacaoServiceImpl
{
  xmlNFe: string[];
  constructor(
  ...
  private async trataRetorno(
    ...
    try {
      ...
      const nfeRetornoAutService = new NFCERetornoAutorizacaoService(
        this.environment,
        this.utility,
        this.xmlBuilder,
        this.axios,
  ...
  private gerarXmlNFCEAutorizacao(data: NFe) {
    ...
    const createXML = (NFe: LayoutNFe) => {
      ...
      if (![4, 9].includes(NFe.infNFe.ide.tpEmis)) {
        qrCode = generateQRCodeURLOnline(
          chaveAcesso,
          "2",
      ...
      const urlConsultaNFCe = this.utility.getUrlNFCe(
        "URL-ConsultaNFCe",
        false,
        ""
      );
      ...
      if ([4, 9].includes(NFe.infNFe.ide.tpEmis)) {
        ...
        qrCode = generateQRCodeURLOffline(
          chaveAcesso,
          "2",
          tpAmb,
          diaEmissao,
        ...
        xml2js.parseString(xmlAssinado, (err, result) => {
          if (err) {
            throw new Error(
              "Erro ao parsear o XML para atualização do qrCode:"
            );
          } else {
